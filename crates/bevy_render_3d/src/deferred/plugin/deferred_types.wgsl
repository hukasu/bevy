#define_import_path bevy_render_3d::deferred::types
...
fn pack_24bit_normal_and_flags(octahedral_normal: vec2<f32>, flags: DeferredFlags) -> u32 {
    let unorm1 = u32(saturate(octahedral_normal.x) * U12MAXF + 0.5);
    let unorm2 = u32(saturate(octahedral_normal.y) * U12MAXF + 0.5);
    return (unorm1 & 0xFFFu) | ((unorm2 & 0xFFFu) << 12u) | ((flags.flags & 0xFFu) << 24u);
}
...
fn unpack_flags(packed: u32) -> DeferredFlags {
    return DeferredFlags { flags: (packed >> 24u) & 0xFFu };
}
