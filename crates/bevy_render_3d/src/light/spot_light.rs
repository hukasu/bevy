...
use bevy_math::{ops, Mat4, Vec3, Vec3A};
use bevy_platform_support::prelude::Box;
use bevy_reflect::{prelude::ReflectDefault, Reflect};
use bevy_render::{
    primitives::{Aabb, Frustum, Sphere},
    view::{RenderLayers, ViewVisibility, Visibility, VisibilityClass},
};
use bevy_transform::components::{GlobalTransform, Transform};

use crate::{
    cluster::plugin::{ClusterAssignable, ClusterAssignableCullMethod},
    volumetric_fog::VolumetricLight,
};

use super::{LightVisibilityClass, VisibleMeshEntities};
...
impl ClusterAssignable for SpotLight {
    type Query<'a> = (
        Entity,
        &'a GlobalTransform,
        &'a SpotLight,
        Option<&'a RenderLayers>,
        Option<&'a VolumetricLight>,
        &'a ViewVisibility,
    );
    type Filter = ();
    ...
    fn shadows_enabled(
        ...
        query_data.2.shadows_enabled
    }

    fn volumetric(
        query_data: &<<Self::Query<'_> as bevy_ecs::query::QueryData>::ReadOnly as bevy_ecs::query::QueryData>::Item<'_>,
    ) -> bool {
        query_data.4.is_some()
    }
    ...
    fn cull_method(
        ...
        Some(Box::new(SpotLightCullMethodExtras {
            outer_angle: query_data.2.outer_angle,
        }))
    }
...
struct SpotLightCullMethodExtras {
    outer_angle: f32,
}

impl ClusterAssignableCullMethod for SpotLightCullMethodExtras {
    fn cull(
        &self,
        ...
        let (angle_sin, angle_cos) = ops::sin_cos(self.outer_angle);
        let spot_light_dir = (view_from_world.clone() * transform.back().extend(0.0))
            .truncate()
            .normalize();
        ...
        let spot_light_offset = Vec3::from(center - cluster_aabb_sphere.center);
        let spot_light_dist_sq = spot_light_offset.length_squared();
        let v1_len = spot_light_offset.dot(spot_light_dir);
        ...
        let front_cull = v1_len > cluster_aabb_sphere.radius + range * view_from_world_scale_max;
