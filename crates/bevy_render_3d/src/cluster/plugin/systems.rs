...
use bevy_math::{ops, Mat4, UVec3, Vec2, Vec3, Vec3A, Vec3Swizzles, Vec4, Vec4Swizzles};
use bevy_render::{
    camera::Camera,
...
use crate::{
    cluster::{
        cluster::{Clusters, ExtractedClusterConfig, ViewClusterBindings},
        clusterable_objects::{
            ClusterableObjectAssignmentData, ClusterableObjectKey,
            ExtractedClusterableObjectElement, ExtractedClusterableObjects,
...
pub fn pre_cluster_assignment(
    mut commands: Commands,
    mut global_clusterable_objects: ResMut<GlobalVisibleClusterableObjects>,
    mut cluster_assignments: Option<ResMut<ClusterAssignments>>,
) {
    global_clusterable_objects.entities.clear();
    if let Some(cluster_assignments) = &mut cluster_assignments {
        cluster_assignments.clusterable_objects.clear();
        cluster_assignments.cluster_aabb_spheres.clear();
    } else {
        commands.init_resource::<ClusterAssignments>();
    }
}

pub fn cluster_assignment<const O: u8, T: ClusterAssignable>(
    query: Query<T::Query<'_>, T::Filter>,
    mut cluster_assignments: ResMut<ClusterAssignments>,
) {
    cluster_assignments
        .clusterable_objects
        .extend(query.iter().filter(T::visible).map(|query_data| {
            ClusterableObjectAssignmentData {
                entity: T::entity(&query_data),
                transform: T::transform(&query_data),
                range: T::range(&query_data),
                object_key: ClusterableObjectKey {
                    order: O,
                    shadows_enabled: T::shadows_enabled(&query_data),
                    volumetric: T::volumetric(&query_data),
                },
                render_layers: T::render_layers(&query_data).unwrap_or_default(),
                cull_method: T::cull_method(&query_data),
            }
        }));
}

pub fn sort_cluster_assigned_objects(
    mut cluster_assignments: ResMut<ClusterAssignments>,
    frusta: Query<&Frustum>,
    mut max_clusterable_objects_warning_emitted: Local<bool>,
) {
    if cluster_assignments.clusterable_objects.len() > MAX_UNIFORM_BUFFER_CLUSTERABLE_OBJECTS {
        cluster_assignments
            .clusterable_objects
            .sort_by_cached_key(|clusterable_object| {
                (clusterable_object.object_key, clusterable_object.entity)
            });
        ...
        cluster_assignments
            .clusterable_objects
            .truncate(MAX_UNIFORM_BUFFER_CLUSTERABLE_OBJECTS);
    }
}

// NOTE: Run this before update_point_light_frusta!
pub fn post_cluster_assignment(
    mut commands: Commands,
    mut global_clusterable_objects: ResMut<GlobalVisibleClusterableObjects>,
    mut views: Query<(
        Entity,
    ...
    ) in &mut views
    {
        ...
        if config.dynamic_resizing() {
            ...
            for clusterable_object in &cluster_assignments.clusterable_objects {
                let clusterable_object_sphere = clusterable_object.sphere();
        ...
        cluster_assignments
            ...
            .extend(core::iter::repeat_n(None, cluster_count));
        ...
        >| {
            let ClusterAssignments {
                clusterable_objects,
                cluster_aabb_spheres,
            } = &mut cluster_assignments.as_mut();
                ...
                for z in min_cluster.z..=max_cluster.z {
                    ...
                    for y in min_cluster.y..=max_cluster.y {
                        ...
                        for x in min_x..=max_x {
                            let culled = if let Some(cull_method) = &clusterable_object.cull_method
                            {
                                ...
                                let center = view_clusterable_object_sphere.center;
                                let range = clusterable_object.range;
                                cull_method.cull(
